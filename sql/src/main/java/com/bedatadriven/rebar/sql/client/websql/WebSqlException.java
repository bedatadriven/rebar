...
public final class WebSqlException extends SqlException {
   ...
   * expected version of the Database or DatabaseSync object, or the Database.changeVersion() or 
   * DatabaseSync.changeVersion() methods were passed a version that doesn't match the actual
   *  database version.
   */
  ...
  /**
   * The statement failed because there was not enough remaining storage space, or the storage quota 
   ...
   * ? placeholders in the statement, or the statement tried to use a statement that is not allowed, such as 
   * BEGIN, COMMIT, or ROLLBACK, or the statement tried to use a verb that could modify the database but the 
   * transaction was read-only.
   */
  public static int SYNTAX_ERR = 5;
  
  /**
   * An INSERT, UPDATE, or REPLACE statement failed due to a constraint failure. For example, because a row 
   * was being inserted and the value given for the primary key column duplicated the value of an existing row.
   */

 ...
 * @author Alex Bertram
 */
class GearsStatement implements Statement {

  private final GearsConnection conn;
  ...
  GearsStatement(GearsConnection connection, Database database) {
    this.database = database;
    this.conn = connection;
  }

  GearsStatement(GearsConnection connection, Database database, String sql) {
    this.conn = connection;
    this.database = database;
    this.sql = sql;
  }
  ...
  protected com.google.gwt.gears.client.database.ResultSet doExecute() throws DatabaseException {
    Log.debug("Statement: Executing SQL: " + sql);
  ...
  protected ResultSet doExecuteQuery() throws SQLException {
    try {
      this.updateCount = -1;
      this.primaryKey = 0;
      this.rs = new GearsResultSet(this, doExecute());
      return rs;
  ...
  public ResultSet getGeneratedKeys() throws SQLException {
    return new KeyResultSet(this.primaryKey);
  }

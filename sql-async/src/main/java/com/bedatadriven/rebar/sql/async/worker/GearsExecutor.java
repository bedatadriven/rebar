...
import com.bedatadriven.rebar.sql.async.client.PreparedStatementBatch;
...
public class GearsExecutor {
  ...
  private int execute() throws Exception {
    try {
      openConnection();
      beginTransaction();
      int rowsAffected = executeUpdates();
      commitTransaction();
      return rowsAffected;
    } catch(Exception e) {
      rollbackSavePoint();
  ...
  private int executeUpdates() throws DatabaseException {
    int rowsAffectedCount = 0;
    for(int i = 0; i!=cmd.getOperations().length(); ++i) {
      PreparedStatementBatch stmt = cmd.getOperations().get(i);
      if(stmt.getExecutions().length()==0) {
        database.execute(stmt.getStatement());
      } else {
        for (int j = 0; i != stmt.getExecutions().length(); ++i) {
          execute(database, stmt.getStatement(), stmt.getExecutions().get(0));

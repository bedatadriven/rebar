...
<#macro setParametersFromProperty value property>
    <#if property.embedded == true>
        ...
        if(${value} == null) {
            ...
            <#list property.embeddedClass.properties as embeddedProperty>
                stmt.${embeddedProperty.column.stmtSetter}(${index}, ${embeddedProperty.convertor}(${value}.${embeddedProperty.getterName}()));
                <#assign index = index + 1>
            </#list>
        }
    <#elseif property.toOne == true>
        ...
        if(${value} == null) {
            ...
            stmt.${property.column.stmtSetter}(${index}, ${property.relatedEntity.id.convertor}(${value}.${property.relatedEntity.id.getterName}()));
        }
        <#assign index = index + 1>
    ...
    <#elseif property.primitive == true>
        ...
        stmt.${property.column.stmtSetter}(${index}, ${property.convertor}(${value}));
...
<#macro putColumnValuesFromEntity entity property>
    <@putColumnValuesFromProperty value=(entity + "." + property.getterName + "()") property=property/>
</#macro>
...
<#macro putColumnValuesFromProperty value property>
    <#if property.embedded == true>
        ...
        if(${value} == null) {
            <#list property.embeddedClass.properties as embeddedProperty>
                map.put("${embeddedProperty.column.name}", null);
            </#list>
        } else {
            <#list property.embeddedClass.properties as embeddedProperty>
                map.put("${embeddedProperty.column.name}", ${value}.${embeddedProperty.getterName}());
            </#list>
        }
    <#elseif property.toOne == true>

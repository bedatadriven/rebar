...
class ${delegateClass}
   ...
   /**
    ...
    static Map<String,Object> getColumnMap(Object entityObject) {
        ${qualifiedClassName} entity = (${qualifiedClassName})entityObject;
            ...
            <#list properties as property>
                <#if property.id == false>
                    if("${property.name}".equals(p.getKey())) {
                        ${property.type.qualifiedBoxedName} value = (${property.type.qualifiedBoxedName})p.getValue();
                        <@putColumnValuesFromProperty "value", property/>
                    }
                </#if>
            </#list>
        }

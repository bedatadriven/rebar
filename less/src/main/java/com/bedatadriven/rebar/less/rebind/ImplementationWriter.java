...
public class ImplementationWriter {
	...
	public ImplementationWriter(GeneratorContext context, JClassType interfaceType, String generatedSimpleSourceName,
			PrintWriter pw) {
	...
	private void writeEnsureInjected() {
		...
		sw.indent();
		sw.println("return com.bedatadriven.rebar.less.client.LessResourceInjector.ensureInjected();");
		sw.outdent();
		sw.println("}");
	}

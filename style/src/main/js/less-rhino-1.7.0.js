...
function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    parser.parse(input, function (e, root) {
        if (e) {
            throw formatError(e, options);
        }
        try {
            logger.info("Finished parsing " + sheet);
...
less.Parser.fileLoader = function (file, currentFileInfo, callback, env) {
    ...
    var endOfPath = Math.max(href.lastIndexOf('/'), href.lastIndexOf('\\')),
        path = href.slice(0, endOfPath + 1);
    ...
    logger.info("Resolved file " + file + " to " + href);
    ...
    try {
        ...
        logger.warn("Could not load file: " + href);
...
// ADAPTED FOR REBAR-STYLE
function apply (context) {

    logger = context;

    logger.info("LESS compiler starting...");
    ...
    var name = "" + context.sourceFileName;

    if (options.sourceMap === true) {
        logger.info("output: " + output);
        if (!output && !sourceMapFileInline) {
            throw "the sourcemap option only has an optional filename if the css filename is given";
        }
    ...
    if (!name) {
        throw 'No files present in the fileset';
    }
